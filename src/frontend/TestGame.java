...
public class TestGame extends Application {
  ...
  private void initHexagons() {
    ...
    // Fourth Row 4 tiles
    tiles[12] =
        new Hexagon(tiles[3].getCenterX(), tiles[7].getCenterY() + TILE_RADIUS + (TILE_RADIUS / 2));
    ...
    tiles[16] =
        new Hexagon(
            tiles[0].getCenterX(), tiles[12].getCenterY() + TILE_RADIUS + (TILE_RADIUS / 2));
  ...
  private void updateRoadColor(int playerNumber, int slot1, int slot2) {
    ...
    for (Hexagon tile : tiles) {
      for (Line line : tile.getEdges()) {
        ...
        if (((vertex1 == slot1) && (vertex2 == slot2))
            || ((vertex1 == slot2) && (vertex2 == slot1))) {
