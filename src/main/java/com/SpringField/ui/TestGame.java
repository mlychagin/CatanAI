...
public class TestGame extends Application {
    ...
    private void drawSettlements(GraphicsContext gc) {
        /*
         * int size = 20; for (int i = 0; i < scenario.root.vertices.length; i++) { VertexNode node =
         * scenario.root.vertices[i]; if (node.city.getSecond() > 0) { Point location = findPoint(i); switch
         * (node.city.getFirst()) { case 0: gc.setFill(Color.RED); break; case 1: gc.setFill(Color.BLUE); break; case 2:
         * gc.setFill(Color.ORANGE); break; case 3: gc.setFill(Color.GREEN); break; } gc.fillRect(location.getX() -
         * (size / 2), location.getY() - size / 2, size, size); gc.setFill(Color.BLACK); } }
         */
    }
    ...
    private void updateRoads() {
        /*
         * for (int i = 0; i < scenario.root.vertices.length; i++) { // i is also the Node # VertexNode node =
         * scenario.root.vertices[i]; for (MutablePair pair : node.listEdges) { if (pair.getFirst() != -1) { // There is
         * a road on edge updateRoadColor(pair.getFirst(), i, pair.getSecond()); } } }
         */
    }
